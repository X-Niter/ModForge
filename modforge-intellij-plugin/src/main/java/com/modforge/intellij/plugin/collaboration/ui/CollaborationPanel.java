...
public class CollaborationPanel extends JBPanel<CollaborationPanel> implements Disposable, CollaborationListener {
    ...
    public CollaborationPanel(@NotNull Project project) {
        ...
        collaborationService.addListener((CollaborationListener) this);
    ...
    private void updateParticipantsList() {
        // Map ParticipantInfo to Participant
        List<Participant> participants = collaborationService.isConnected()
                ? collaborationService.getParticipants().values().stream()
                        .map(info -> new Participant("placeholderUserId", info.getUsername(), info.isHost()))
    ...
    private void joinSession(ActionEvent e) {
        ...
        future.thenAccept(success -> {
            SwingUtilities.invokeLater(() -> {
                if (success) {
                    updateUI();
                    showInfoDialog("Successfully joined session.", "Success");
                } else {
    ...
    private void leaveSession(ActionEvent e) {
        ...
        future.thenAccept(success -> {
            SwingUtilities.invokeLater(() -> {
                if (success) {
                    updateUI();
                    showInfoDialog("Successfully left session.", "Success");
                } else {
    ...
    @Override
    public void dispose() {
        collaborationService.removeListener((CollaborationListener) this);
    }
