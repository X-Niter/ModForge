...
public class CollaborationWebSocketHandler implements WebSocketMessageListener {
    ...
    public void onMessageReceived(
            @NotNull com.modforge.intellij.plugin.collaboration.websocket.WebSocketMessage message) {
        executor.submit(() -> {
            try {
    ...
    private void handleJoinMessage(@NotNull Map<String, Object> data) {
        ...
        // Add participant
        collaborationService.addParticipant(userId, username);
    }
    ...
    private void handleOperationMessage(@NotNull Map<String, Object> data) {
        String sessionId = (String) data.get("sessionId");
        String userId = (String) data.get("userId");
        Object operationObj = data.get("operation");

        if (sessionId == null || userId == null || !(operationObj instanceof Map)) {
            LOG.warn("Invalid operation message: " + data);
            return;
        }

        @SuppressWarnings("unchecked")
        Map<String, Object> operationData = (Map<String, Object>) operationObj;
