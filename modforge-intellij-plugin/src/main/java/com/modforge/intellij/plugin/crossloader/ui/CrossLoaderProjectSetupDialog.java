...
public class CrossLoaderProjectSetupDialog extends DialogWrapper {
    ...
    public CrossLoaderProjectSetupDialog(@NotNull Project project) {
        ...
        this.architecturyService = project.getService(ArchitecturyService.class);
    ...
    protected ValidationInfo doValidate() {
        ...
        if (!modIdField.getText().matches("[a-z0-9_]+")) {
            return new ValidationInfo("Mod ID must contain only lowercase letters, numbers, and underscores",
                    modIdField);
        }
    ...
    private void createArchitecturyProject() {
        ...
        try {
            ...
            // Validate file chooser selection
            ValidationInfo validationInfo = validateFileChooserSelection(baseDir);
            if (validationInfo != null) {
                Messages.showErrorDialog(project, validationInfo.message, "Invalid Selection");
                return;
            }

            // Generate Architectury project using the service
            if (architecturyService != null) {
                CrossLoaderProjectConfig config = getConfiguration();
                boolean success = architecturyService.generateArchitecturyProject(config, baseDir);
                ...
                if (success) {
                    ...
                    LOG.error("Failed to generate project: " + modId);
                    Messages.showErrorDialog(
                            project,
                            "Failed to generate project. See the IDE log for details.",
                            "Project Creation Failed");
                }
            } else {
                LOG.error("Failed to get ArchitecturyService for project: " + project.getName());
                Messages.showErrorDialog(
                        project,
                        "Failed to initialize project service. See the IDE log for details.",
                        "Project Creation Failed");
            }
        } catch (Exception e) {
            LOG.error("Failed to generate Architectury mod project: " + modId, e);
            Messages.showErrorDialog(
                    project,
                    "Failed to create cross-loader mod project. Error: " + e.getMessage(),
                    "Project Creation Failed");
        }
    }

    /**
     * Validates a selected directory for project creation.
     *
     * @param directory The directory to validate
     * @return ValidationInfo if there's an error, null if valid
     */
    @Nullable
    private ValidationInfo validateFileChooserSelection(@NotNull VirtualFile directory) {
        if (!directory.isDirectory()) {
            return new ValidationInfo("Please select a folder, not a file");
        }

        if (!directory.isWritable()) {
            return new ValidationInfo("The selected folder is not writable");
        }
        ...
        if (children != null && children.length > 0) {
            int result = Messages.showYesNoDialog(
                    project,
                    "Selected folder is not empty. Files may be overwritten. Continue?",
                    "Warning",
                    Messages.getWarningIcon());
            if (result != Messages.YES) {
                return new ValidationInfo("Please select an empty folder");
            }
        }
    ...
    /**
     * Creates a FileChooserDescriptor for selecting a project folder.
     */
    private static FileChooserDescriptor createFolderDescriptor() {
        return FileChooserDescriptorFactory.createSingleFolderDescriptor()
                .withTitle("Select Project Location")
                .withDescription("Choose a location for your new mod project")
    ...
    private void createDirectConversionProject() {
        LOG.info("Direct conversion project creation is not yet implemented.");
        Messages.showInfoMessage(
                project,
                "Direct conversion project creation is currently not supported.",
