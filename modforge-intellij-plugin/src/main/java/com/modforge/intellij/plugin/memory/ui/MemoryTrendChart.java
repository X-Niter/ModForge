...
public class MemoryTrendChart extends JPanel {
     ...
     * Set the chart type
     *
     * @param type The chart type (0=Used Memory, 1=Memory Usage %, 2=Free Memory,
     *             3=All Types)
     */
            ...
            synchronized (displaySnapshots) {
                ...
                if (timeRangeMinutes > 0) {
                    ...
                    for (MemorySnapshot snapshot : snapshots) {
                        if (snapshot.getTimestampDateTime().isAfter(cutoffTime)) {
    ...
    private void drawGrid(Graphics2D g2d, Rectangle2D chartArea, long minValue, long maxValue) {
        ...
        if (displaySnapshots.size() > 1) {
            LocalDateTime firstTime = displaySnapshots.get(0).getTimestampDateTime();
            LocalDateTime lastTime = displaySnapshots.get(displaySnapshots.size() - 1).getTimestampDateTime();
    ...
    private void drawLabels(Graphics2D g2d, Rectangle2D chartArea, long minValue, long maxValue) {
        ...
        for (int i = 0; i <= numYLabels; i++) {
            ...
            g2d.drawString(label, (int) (chartArea.getMinX() - fm.stringWidth(label) - 5),
     ...
     */
    private void drawDataLine(Graphics2D g2d, Rectangle2D chartArea, int dataType, long minValue, long maxValue,
            Color color) {
        ...
        for (MemorySnapshot snapshot : displaySnapshots) {
            double x = calculateXPosition(snapshot.getTimestampDateTime(), firstTime, lastTime, chartArea);
     ...
     */
    private double calculateXPosition(LocalDateTime timestamp, LocalDateTime firstTime, LocalDateTime lastTime,
     ...
     */
    private double calculateYPosition(int dataType, MemorySnapshot snapshot, long minValue, long maxValue,
            Rectangle2D chartArea) {
        double value;
     ...
     * @param maxValue  The maximum value
     */
    private void drawHoverInfo(Graphics2D g2d, Rectangle2D chartArea, Point point, MemorySnapshot snapshot,
        ...
        Date date = Date.from(snapshot.getTimestampDateTime().atZone(ZoneId.systemDefault()).toInstant());
            ...
            for (MemorySnapshot snapshot : displaySnapshots) {
                ...
                if (distance < nearestDistance) {
                    ...
                    if (chartType == 3) { // All types - use used memory for hover
                        ...
                        y = calculateYPosition(chartType == 2 ? 2 : chartType, snapshot, 0, determineMaxValue(),
                                chartArea);
                    }
